...
class PaymentRepo
{
    ...
    //فروش دوره های هر مدرس
    public function getUserSuccessPayments($userId)
    {
        return Payment::query()->where('seller_id',$userId)->where('status',Payment::STATUS_SUCCESS);
    }
    ...
    {
        return $this->getUserSuccessPayments($userId)->sum('seller_id');
    }
    ...
    {
        return $this->getUserSuccessPayments($userId)->whereDate('created_at','<=',$startDate)
